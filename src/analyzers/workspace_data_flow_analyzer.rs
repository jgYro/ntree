...
use crate::models::{
    ControlFlowGraph, DataFlowGraph, DecisionTreeSet, DefUseChainSet, VariableLifecycleSet,
};
use crate::storage::{
    FileRecord, InterproceduralCFG, NameResolver, ProjectDetector, ProjectInfo, SymbolStore,
};
...
impl WorkspaceDataFlowAnalyzer {
    ...
    pub fn analyze_workspace(
        ...
        for project in &projects {
            let project_result =
                self.analyze_project(project, symbol_store, name_resolver, interprocedural_cfg)?;

            // Merge results
            all_data_flow_graphs.extend(project_result.data_flow_graphs);
            self.merge_variable_lifecycles(
                &mut workspace_variable_lifecycles,
                project_result.variable_lifecycles,
            );
    ...
    fn analyze_project(
        ...
        let project_def_use = DefUseChainSet::new();
        let project_decisions = DecisionTreeSet::new();

        // Use symbol store to get function information
        let function_symbols = symbol_store.find_symbols_by_name("");
        let total_symbols = function_symbols.len();

        // Use name resolver to resolve cross-file dependencies
        let import_mappings = name_resolver.get_import_mappings();
        let has_imports = !import_mappings.is_empty();

        // Use interprocedural CFG for cross-function analysis
        let interproc_edges = interprocedural_cfg.get_interprocedural_edges();
        let function_count = interproc_edges.len();

        // Log analysis context
        if total_symbols > 0 || has_imports || function_count > 0 {
            // Context available for enhanced analysis
        }
            ...
            if let Ok(cfgs) = crate::api::analysis::generate_cfgs(&file_record.path) {
                for cfg_result in cfgs {
                    ...
                    if let Some(cfg) = self.convert_cfg_result_to_control_flow_graph(&cfg_result) {
                        ...
                        if let Ok(data_flow) = self
                            .data_flow_analyzer
                            .analyze_function(&cfg_result.function_name, &cfg)
                        {
        ...
        let cross_file_vars =
            self.detect_cross_file_variables(&project.source_files, name_resolver)?;
    ...
    fn convert_cfg_result_to_control_flow_graph(
        &self,
        _cfg_result: &crate::api::analysis::CfgResult,
    ...
    fn merge_variable_lifecycles(
        &self,
        workspace_set: &mut VariableLifecycleSet,
        project_set: VariableLifecycleSet,
    ) {
    ...
    fn merge_def_use_chains(
        &self,
        workspace_set: &mut DefUseChainSet,
        project_set: DefUseChainSet,
    ) {
    ...
    fn merge_decision_trees(
        &self,
        workspace_set: &mut DecisionTreeSet,
        project_set: DecisionTreeSet,
    ) {
