...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for constructor in exact_constructors.iter().take(5) {
        println!("  {} in {}", constructor.name,
                constructor.file_path.file_name().map(|n| n.to_string_lossy()).unwrap_or("unknown".into()));
    }
    ...
    for getter in getters.iter().take(3) {
        println!("  {} in {}", getter.name,
                getter.file_path.file_name().map(|n| n.to_string_lossy()).unwrap_or("unknown".into()));
    }
